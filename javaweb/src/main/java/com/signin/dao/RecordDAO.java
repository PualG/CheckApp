...
public class RecordDAO implements IRecordDAO {
    ...
    protected static String SELECT_SQL="select " + FIELDS_INSERT + " from "+ TBALE_NAME + " where name = ?  and temp = ? and temp1 = ? and temp6 = ?";
    protected static String UPDATE_SQL="update "+ TBALE_NAME + " set date_time = ?, name = ?, temp = ?, temp1 = ?, temp2 = ?, temp3 = ?, temp4 = ?, temp8 = ?, temp5 = ?, location = ?, temp6 = ?, temp9 = ?, temp7 = ?"+" where name = ?and date_time= ?";
    protected static String DELETE_SQL ="delete from "+ TBALE_NAME + " where name = ? and date_time = ?";
    protected static String SELECT_SQL2="select " + FIELDS_INSERT + " from "+ TBALE_NAME + " where name = ? and date_time = ?";
    ...
    public Record select(Record record) throws Exception {
        ...
        try {
            con = DbConnect.getDBconnection();
            prepStmt = con.prepareStatement(SELECT_SQL2);
    ...
    public List<Record> select1(Record record) throws Exception {
        Connection con = null;
        PreparedStatement prepStmt = null;
        ResultSet rs = null;
        ...
        try {
            con = DbConnect.getDBconnection();
            String s="";
            if(record.getName().isEmpty()){s +="name is not null ";}
            else {s +="name='"+record.getName()+"'";}
            if(record.getTemp().isEmpty()){s +="and temp is not null ";
            }else {if(record.getTemp().equals("正常")){
                s +="and temp='正常'";
            }else { s +="and temp!='正常'";}}
            if(record.getTemp1().isEmpty()){s +="and temp1 is not null ";
            }else {if(record.getTemp1().equals("无")){
                s +="and temp1='无'";
            }else { s +="and temp1!='无'";}}
            if(record.getTemp6().isEmpty()){s +="and temp6 is not null ";
            }else {s +="and temp6='"+ record.getTemp6()+"'";}
            System.out.println(record.getName());
            prepStmt = con.prepareStatement("select * from signin_table where  "+s+ " order by date_time desc");
            System.out.println(prepStmt);
            rs = prepStmt.executeQuery();
    ...
    public int update(Record record) throws Exception {
        ...
        try {
            ...
            prepStmt.setString(12, record.getTemp9());
            prepStmt.setString(13, record.getTemp7());
            prepStmt.setString(14, record.getDate_time1());
            prepStmt.setString(15, record.getName1());
            System.out.println(prepStmt);
            result = prepStmt.executeUpdate();
        } catch (Exception e) {
            // handle exception
